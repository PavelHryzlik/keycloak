...
import org.keycloak.saml.EidasSAMLRequestParser;
...
import org.keycloak.saml.processing.core.saml.v2.util.EidasAssertionUtil;
 ...
 * @version $Revision: 1 $
 */
public class EidasSAMLEndpoint  {
    protected static final Logger logger = Logger.getLogger(EidasSAMLEndpoint.class);
    ...
    protected abstract class Binding {
                ...
                if (assertionIsEncrypted) {
                    try {
                        ...
                        /* End of deprecated code */
                            assertionElement = EidasAssertionUtil.decryptAssertion(responseType, new SAMLDecryptionKeysLocator(session, realm, config.getEncryptionAlgorithm())); 
                        }
        ...
        private AuthenticationSessionModel samlIdpInitiatedSSO(final String clientUrlName) {
            ...
            Optional<ClientModel> oClient = EidasSAMLEndpoint.this.session.clients()
            ...
            SamlService samlService = (SamlService) factory.createProtocolEndpoint(EidasSAMLEndpoint.this.session, event);
            AuthenticationSessionModel authSession = samlService.getOrCreateLoginSessionForIdpInitiatedSso(session, EidasSAMLEndpoint.this.realm, oClient.get(), null);
            if (authSession == null) {
    ...
    protected class PostBinding extends Binding {
        ...
        protected SAMLDocumentHolder extractRequestDocument(String samlRequest) {
            return EidasSAMLRequestParser.parseRequestPostBinding(samlRequest);
        }
        @Override
        protected SAMLDocumentHolder extractResponseDocument(String response) {
            byte[] samlBytes = PostBindingUtil.base64Decode(response);
            return EidasSAMLRequestParser.parseResponseDocument(samlBytes);
        }
    ...
    protected class RedirectBinding extends Binding {
        ...
        @Override
        protected SAMLDocumentHolder extractRequestDocument(String samlRequest) {
            return EidasSAMLRequestParser.parseRequestRedirectBinding(samlRequest);
        }

        @Override
        protected SAMLDocumentHolder extractResponseDocument(String response) {
            return EidasSAMLRequestParser.parseResponseRedirectBinding(response);
        }
