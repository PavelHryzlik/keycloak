...
public class EidasSAMLAttributeParser extends AbstractStaxSamlAssertionParser<AttributeType> {

    private static final EidasSAMLAttributeParser INSTANCE = new EidasSAMLAttributeParser();
    ...
    private EidasSAMLAttributeParser() {
        super(SAMLAssertionQNames.ATTRIBUTE);
    }

    public static EidasSAMLAttributeParser getInstance() {
        return INSTANCE;
    }
    ...
    protected void processSubElement(XMLEventReader xmlEventReader, AttributeType target, SAMLAssertionQNames element, StartElement elementDetail) throws ParsingException {
        switch (element) {
            case ATTRIBUTE_VALUE:
                target.addAttributeValue(EidasSAMLAttributeValueParser.getInstance().parse(xmlEventReader));
                break;
